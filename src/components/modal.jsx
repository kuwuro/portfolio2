    ...
    const modalContent = (
        screenWidth <= 600 ? (
            <div className={`fixed bottom-0 w-full z-50 transition-opacity ${modalOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} style={{ height: '80vh' }}>
                <div className="modal-content max-w-full md:max-w-lg lg:max-w-none w-auto h-full flex flex-col p-8 bg-white dark:bg-black rounded-lg rounded-b-none shadow-xl dark:border-2 dark:border-b-0 dark:border-white20 border-opacity-20 bg-opacity-90 backdrop-blur-xl transition-transform transform scale-100 fade-up">
                    {children}
